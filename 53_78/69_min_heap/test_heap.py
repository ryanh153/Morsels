...
class MinHeapTests(unittest.TestCase):
    ...
    def test_faster_than_sorting(self):
        many_big_numbers = [randint(100, 1000) for n in range(3000)]
        heap = MinHeap(many_big_numbers)
        time = partial(timeit, globals=locals(), number=30)

        sort_timer = time("items = sorted(many_big_numbers)[:8]")
        min_heap_timer = time(dedent("""
            items = [heap.pop() for _ in range(8)]
        """))
        print(sort_timer)
        print(min_heap_timer)
        self.assertLess(min_heap_timer, sort_timer)
